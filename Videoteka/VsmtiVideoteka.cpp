...
void VsmtiVideoteka::GledateljiBezFilma()
{
		cout << "***************Gledatelji bez filma******************************** " << endl;
		for (size_t i = 0; i < vGledatelji.size(); i++)
		{
			for (size_t j = 0; j < vGledateljiNaFilmu.size(); j++)
			{
				if (vGledatelji[i]->m_nOibGledatelja != vGledateljiNaFilmu[j]->m_oGledatelj->m_nOibGledatelja) {
					cout << "OIB: " << vGledatelji[i]->m_nOibGledatelja << " IME: " << vGledatelji[i]->m_sIme << " PREZIME: " << vGledatelji[i]->m_sPrezime << "GODINA RODJENJA: " << vGledatelji[i]->m_nGodinaRodjenja << " SIFRA FILMA: NEMA FILM" << endl;
				}
			}
		}
	}
...
{
	cout << "\n";
	cout << "GLEDATELJI: " << endl;
	for (size_t i = 0; i < vGledatelji.size(); i++)
	{
		cout << "OIB: " << vGledatelji[i]->m_nOibGledatelja <<" IME: " << vGledatelji[i]->m_sIme << " PREZIME: " << vGledatelji[i]->m_sPrezime<<" GODINA RODJENJA: "<<vGledatelji[i]->m_nGodinaRodjenja<< endl;
...
void VsmtiVideoteka::IspisiSveFilmove() {
	...
	cout << "FILMOVI:" << endl;
	for (size_t i = 0; i < vFilmovi.size(); i++) {
		cout <<"Rbr: "<<to_string(rbr)+"."<< " ZANR: " << vFilmovi[i]->m_oZanr->m_sNazivZanra << " NAZIV: " << vFilmovi[i]->m_sNazivFilma << " OPIS:" << vFilmovi[i]->m_sOpisFilma << " SIFRA: " << vFilmovi[i]->m_nSifra << endl;
		rbr++;
	}
}
...
void VsmtiVideoteka::GledateljiNaFilmovima() {
	...
	int rbr = 1;
	for (size_t i = 0; i < vGledateljiNaFilmu.size(); i++) {
		if (sifra == vGledateljiNaFilmu[i]->m_oFilm->m_nSifra) {
	...
	if (stanje==true) {
		for (size_t j = 0; j < gledatelji.size(); j++) {
			cout << "Rbr: " << to_string(rbr) + ".";
			cout << " OIB GLEDATELJA: " << gledatelji[j].m_oGledatelj->m_nOibGledatelja << " IME i PREZIME: " << gledatelji[j].m_oGledatelj->m_sIme << " " << gledatelji[j].m_oGledatelj->m_sPrezime << endl;
			rbr++;
		}
	}
...
void VsmtiVideoteka::PrikaziSveIzdaneFilmove() {
	...
	for (size_t k = 0; k < vFilmovi.size(); k++) {
		...
		if (br != 0) {		
					cout <<"Rbr: "<<to_string(rbr)+"."<< " NAZIV FILMA: " << vFilmovi[k]->m_sNazivFilma << " GLEDATELJI: " << temp<<endl ;
					rbr++;	
		}
		gledatelji.clear();
	}
		...
		for (size_t j = 0; j < vFilmoviOpet.size(); j++) {
			...
			if (vFilmoviOpet[j].m_sNazivLowerCaseFilma.find(pojam) != string::npos || vFilmoviOpet[j].m_sNazivLowerCaseZanra.find(pojam) != string::npos) {
				...
				if (vFilmovi[i]->m_nSifra == vFilmoviOpet[j].m_sIstaSifraFilma) {
					cout << "Rbr: " << to_string(rbr) + "." << " NAZIV: " << vFilmovi[i]->m_sNazivFilma << " OPIS: " << vFilmovi[i]->m_sOpisFilma << " ZANR: " << vFilmovi[i]->m_oZanr->m_sNazivZanra << " SIFRA FILMA: " << vFilmovi[i]->m_nSifra << endl;
					rbr++;
					}
				}	
			}	
...
void VsmtiVideoteka::IzdavanjeFilma() {
	...
	if (oibGledatelja.length() < 5 || oibGledatelja.length() > 6) {
	...
	ponovniUnos:
		...
		cout << "----------------------Unosenje sifre filma-----------------------" << endl;
		IspisiSveFilmove();
	unosFilma:
		cout << "Sifra filma:";
...
unosSifreFilma:
	...
	if (sifraFilma.length() < 3 || sifraFilma.length() > 4) {
...
void VsmtiVideoteka::IspisiSveZanrove() {
	cout << "--------------------ZANROVI----------------------" << endl;
	for (size_t i = 0; i < vZanrovi.size(); i++) {
		cout << "ID: " << vZanrovi[i]->m_nIdZanra << " NAZIV: " << vZanrovi[i]->m_sNazivZanra << endl;
	}
}
		...
		for (size_t j = 0; j < vLowerCaseGledatelji.size(); j++) {
			...
			if (vLowerCaseGledatelji[j].m_sImeLowerCase.find(pojam) != string::npos || vLowerCaseGledatelji[j].m_sPrezimeLowerCase.find(pojam) != string::npos) {
				...
				if (vGledatelji[i]->m_nOibGledatelja == vLowerCaseGledatelji[j].m_sOib) {
					cout << "Rbr: " << to_string(rbr) + "." << " IME: " << vGledatelji[i]->m_sIme << " PREZIME: " << vGledatelji[i]->m_sPrezime << " GODINA RODJENJA: " << vGledatelji[i]->m_nGodinaRodjenja<< " OIB: " << vGledatelji[i]->m_nOibGledatelja << endl;
						...
						for (l = 0; l < vOibi.size(); l++) {
							if (vGledateljiNaFilmu[ii]->m_oGledatelj->m_nOibGledatelja == vOibi[l].m_sOib) {
								if (vOibi[l].m_nRbr == stoi(a)) {
									...
									cout << "Rbr: " << to_string(rbr2) + "." << " ZANR: " << vGledateljiNaFilmu[ii]->m_oFilm->m_oZanr->m_sNazivZanra << " NAZIV: " << vGledateljiNaFilmu[ii]->m_oFilm->m_sNazivFilma << " OPIS: " << vGledateljiNaFilmu[ii]->m_oFilm->m_sOpisFilma << " SIFRA: " << vGledateljiNaFilmu[ii]->m_oFilm->m_nSifra << endl;
									rbr2++;
									gledateljFilma = true;
									prijelaz=1;
								}
